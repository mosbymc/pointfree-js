...
describe('Test functional helpers', function _testFunctionalHelpers() {
    ...
    describe('Test lessThan', function _testLessThan() {
        it('should return the correct boolean response given the two arguments', function _testLessThan() {
            lessThan(1, 2).should.be.true;
            lessThan(2, 1).should.be.false;
            lessThan('1', '2').should.be.true;
            lessThan('2', '1').should.be.false;
            lessThan(1, '1').should.be.false;
            lessThan('1', 1).should.be.false;
            lessThan(false, true).should.be.true;
        });
    });

    describe('Test lessThanOrEqual', function _testLessThanOrEqual() {
        it('should return true if the first argument is less than or equal to the second', function _testLessThanOrEqual() {
            lessThanOrEqual(1, 2).should.be.true;
            lessThanOrEqual(1, 1).should.be.true;
            lessThanOrEqual(2, 1).should.be.false;
            lessThanOrEqual('1', '1').should.be.true;
            lessThanOrEqual('1', '2').should.be.true;
            lessThanOrEqual('2', '1').should.be.false;
        });
    ...
    describe('Test mapSet', function _testMapSet() {
        it('should create a new map with all the same values except the updated one and return it', function _testMapSet() {
            let m = new Map();
            m.set(1, 1);
            m.set(2, 2);
            m.set(3, 3);

            let res = mapSet(2, 4, m);

            m.get(2).should.eql(2);
            res.get(2).should.eql(4);
        });
    });

    describe('Test modulus', function _testModulus() {
        it('should return the result of modulating the first argument with the second', function _testModulus() {
            modulus(2, 1).should.eql(0);
    ...
    describe('Test multiply', function _testMultiple() {
        it('should return the result of multiplying the arguments', function _testMultiply() {
            multiply(1, 2).should.eql(2);
            multiply(2, 2).should.eql(4);
            multiply(5, 3.1).should.eql(15.5);
        });
    });

    describe('Test negate', function _testNegate() {
        it('should negate its argument', function _testNegate() {
            negate(1).should.eql(-1);
            negate(-1).should.eql(1);
        });
    });

    describe('Test notEqual', function _testNotEqual() {
        it('should return a boolean indicating the loose inequality of the two arguments', function _testNotEqual() {
            notEqual(1, 1).should.be.false;
            notEqual(1, '1').should.be.false;
            notEqual(2, 1).should.be.true;
            notEqual(null, undefined).should.be.false;
            notEqual(0, '').should.be.false;
        });
    });
    ...
    describe('Test reverse', function _testReverse() {
        ...
        it('should reverse the order of the arguments', function _testReverseWithMultipleArguments() {
            reverse(1, 2, 3, 4, 5).should.eql([5, 4, 3, 2, 1]);
        });
    });

    describe('Test setSet', function _testSetSet() {
        it('should set the property on the object', function _testSetSet() {
            let s = new Set();
            s.add(1);
            s.add('1');
            s.add(2);
    ...
    describe('Test strictNotEqual', function _testStrictNotEqual() {
        it('should return a boolean result based on the strict inequality of the two arguments', function _testStrictNotEqual() {
            strictNotEqual(1, 2).should.be.true;
            strictNotEqual(1, '1').should.be.true;
            strictNotEqual(1, 1).should.be.false;
            strictNotEqual(0, false).should.be.true;
        });
    });

    describe('Test subtract', function _testSubtract() {
        it('should subtract the second number from the first', function _testSubtract() {
            subtract(1, 1).should.eql(0);
            subtract(10, 1).should.eql(9);
            subtract(5, 7).should.eql(-2);
        });
    });
