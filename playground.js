...
//TODO:
//TODO:         - In the interest of allowing for easy extensibility, I should probably wrap each deferred execution 'method's' returned iterator
...
function sort(item, source, keySelector) {
    var low = 0,
    ...
    while (high >= low) {
        if (itemKey < keySelector(source[mid])) {
            high = mid - 1;
            mid = parseInt(((low + high) / 2) + 0.5);
        }
        else if (itemKey > keySelector(source[mid])) {
            low = mid + 1;
            mid = parseInt(((low + high) / 2) + 0.5);
        }
        else break;
    }
    return source.slice(0, mid).concat(item).concat(source.slice(mid));
}
...
function attachListener(evt) {
    ...
    iterator.next();
    document.addEventListener(evt, function _tester(e) {
        console.log(iterator.next(e).value);
    });
}
