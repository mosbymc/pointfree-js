 ...
 * source,
 * source,
 * map: {function} queryable_core._map,
 * groupBy: {function} queryable_core._groupBy,
 * groupByDescending: {function} queryable_core._groupByDescending,
 * flatten: {function} queryable_core._flatten,
...
internal_orderedQueryable.thenBy = function _thenBy(keySelector, comparer) {
    var sortObj = this._appliedSorts.concat({ keySelector: keySelector, direction: 'asc' });
    return createNewOrderedQueryableDelegator(this.source, orderBy(this, sortObj), sortObj, comparer);
};
